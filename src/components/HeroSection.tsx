...
const HeroSection = () => {
  ...
  return (
    <section id="hero" className="min-h-screen flex items-center relative overflow-hidden bg-background pt-[120px] pb-[60px] px-6 md:px-[60px]">
      ...
      {/* Background text */}
      <div 
        className="absolute font-heading text-[clamp(80px,18vw,260px)] text-primary/5 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap select-none pointer-events-none animate-bgdrift z-0"
      ...
      >
        ...
        <h1 className="font-heading text-[clamp(42px,10vw,138px)] leading-[0.9] tracking-[2px] mb-[22px]">
        ...
        <div ref={statsRef} className="flex gap-6 sm:gap-8 md:gap-14 mb-12 flex-wrap animate-slide-left" style={{ animationDelay: '0.3s' }}>
          {stats.map((stat, i) => (
            <div key={stat.label} className="group cursor-default">
              <div className="font-heading text-[2.5rem] sm:text-[3rem] md:text-[3.5rem] text-primary leading-none transition-transform group-hover:scale-110 group-hover:text-foreground">
                {counts[i]}
              </div>
        ...
        <div className="flex gap-5 flex-wrap animate-slide-left" style={{ animationDelay: '0.4s' }}>
          <a href="#contact" className="bg-primary text-foreground px-8 md:px-[50px] py-[15px] md:py-[18px] font-heading text-[1rem] sm:text-[1.2rem] tracking-[4px] no-underline clip-skew btn-shine hover:bg-[#ff1010] hover:-translate-y-[3px] transition-all inline-block">
            Enroll Now
          </a>
          <a href="#semesters" className="border-2 border-foreground text-foreground px-6 md:px-10 py-[13px] md:py-4 font-heading text-[1rem] sm:text-[1.2rem] tracking-[4px] no-underline hover:bg-foreground hover:text-background transition-all inline-block">
            View Curriculum
          </a>
        </div>
      </div>
